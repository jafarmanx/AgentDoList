...
# Automatically detect the curio-arch-web-production container
puts "→ Detecting curio-arch-web-production container..."
container_output, status = Open3.capture2(%(ssh app@curio-arch-app-101 "docker ps --format '{{.Names}}' | grep curio-arch-web-production"))
abort("Failed to detect container") unless status.success?

CONTAINER = container_output.strip
abort("No curio-arch-web-production container found") if CONTAINER.empty?
...
Dir.mktmpdir do |tmpdir|
  ...
  cmd = %(ssh app@curio-arch-app-101 "docker cp #{CONTAINER}:#{REMOTE_PATH} -" | tar -xOf - > #{local_file})
